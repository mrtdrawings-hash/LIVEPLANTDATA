...
def draw_digital_display(value, image_filename, display_type="mw"):
    ...
    try:
        ...
        # High visibility absolute central placement of the digital reading box
        ...
        if display_type == "hz":
            ...
            text_color = (0, 0, 0, 255)  # Sharp high-contrast black text
        else:
        ...
        # --- COORDINATE SYSTEM FOR TOTAL MW DIAL ---
        if display_type == "total":
            ...
            # FIX 1: Harmonized dial pivot center with the true graphics center
            dial_center_x = width * 0.485
            dial_center_y = height * 0.49

            # FIX 2: Smooth linear mapping for a 290-degree uniform dial arc
            # 375 MW matches top dead center (270 degrees in image space)
            angle_deg = 270.0 + (numeric_val - 375.0) * (290.0 / 750.0)
            angle_rad = math.radians(angle_deg)

            # Scaling geometry for drawing the inner-pointing wedge indicator
...
live_panel()import streamlit as st
import requests
import os
import math
from PIL import Image, ImageDraw, ImageFont
