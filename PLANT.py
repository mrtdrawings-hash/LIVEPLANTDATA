...
def draw_digital_display(value, image_filename, display_type="mw"):
    ...
    try:
        ...
        if display_type == "hz": text_color = (255, 235, 0, 255)
        ...
        if display_type == "total":
            ...
            # Linear 240-degree calibration: 0 MW = 240 deg, 750 MW = 0 deg
...
def live_panel():
    try:
        ...
        if response.status_code == 200:
            ...
            # Update slots logic remains the same as your provided original
            u1_val = str(data.get("UNIT1", {}).get("MW", "0"))
            u2_val = str(data.get("UNIT2", {}).get("MW", "0"))
            u3_val = str(data.get("UNIT3", {}).get("MW", "0"))
            total_mw = str(int(float(u1_val) + float(u2_val) + float(u3_val)))
            
            img_total = draw_digital_display(total_mw, "Gemini_T.jpg", display_type="total")
